...
class KoGPT2Chat(LightningModule):
    ...
    def training_step(self, batch, batch_index):
        ...
        out = self(token_ids)
        mask_3d = mask.unsqueeze(dim=2).repeat_interleave(repeats=out.shape[2],
                                                          dim=2)
        mask_out = torch.where(mask_3d == 1, out,
                               self.neg * torch.ones_like(out))
    ...
    def _collate_fn(self, batch):
        ...
        return torch.LongTensor(data), torch.LongTensor(
            mask), torch.LongTensor(label)

    def train_dataloader(self):
        data = pd.read_csv('Chatbot_data/ChatbotData .csv')
        self.train_set = CharacterDataset(data,
                                          self.tok_path,
                                          self.vocab,
